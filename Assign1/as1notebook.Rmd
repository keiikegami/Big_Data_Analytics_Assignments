...
Line_list_LE <- list()
Line_list_LCBD <- list()
Line_list_LNCBD <- list()
Line_list_RBRT <- list()

tmps <- list(tmp_CC, tmp_LE, tmp_LCBD, tmp_LNCBD, tmp_RBRT)
Line_lists <- list(Line_list_CC, Line_list_LE, Line_list_LCBD, Line_list_LNCBD, Line_list_RBRT)
for (i in 1:5){
  for (j in 1:length(tmps[[i]])){
    Line_lists[[i]] <- c(Line_list[[i]], tmps[[i]][j]@lines[[1]]@Lines)
  }
}
